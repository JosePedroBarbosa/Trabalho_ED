...
public class LinkedHeap<T> extends LinkedBinaryTree<T> implements HeapADT<T> {
     ...
     *
     * @return the root node that represent the minimum element in the heap
     */
    @Override
    public T findMin() {
        if (isEmpty()) {
    ...
    public ArrayUnorderedList<T> removeAllElements() {
        ...
        while (!isEmpty()) {
            try {
                removedElements.addToRear(removeMin());
            } catch (EmptyCollectionException e) {
                System.err.println("Error while removing element from the heap: " + e.getMessage());
            }
        }
